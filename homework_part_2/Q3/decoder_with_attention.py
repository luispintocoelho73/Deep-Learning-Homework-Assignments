...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        ...
        #print('encoder_out:',encoder_out.shape, ', encoder_attr:',encoder.shape)

        decoder = self.decoder_att(decoder_hidden).unsqueeze(1)
        #print('decoder:',decoder_hidden.shape,', decoder_attr:',decoder.shape)

        out = self.relu(encoder + decoder)
        #print('out:',out.shape)
    
        p = self.softmax(self.full_att(out))

        attention_weighted_encoding = torch.sum(p*encoder_out,axis=1)
...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, attention_dim, decoder_dim, embed_dim, vocab_size,
    ...
    def forward(self, word, decoder_hidden_state, decoder_cell_state, encoder_out):
        ...
        embedded = self.dropout(self.embedding(word)) # Not necessary
        ...
        decoder_hidden_state, decoder_cell_state = self.decode_step(attention_embedded,(decoder_hidden_state, decoder_cell_state))
